(() => {
  ...
  const STORAGE_KEY = 'gridlandState';
  const ASSET_DB_NAME = 'GridLandAssets';
  ...
  const els = {
    ...
    deletePageEdit: $('#deletePageEdit'),
    edgeLeftIndicator: $('#edgeLeftIndicator'),
    edgeRightIndicator: $('#edgeRightIndicator'),
    addSiteButton: $('#addSiteButton'),
    editLayoutButton: $('#editLayoutButton'),
    columnsSelect: $('#columnsSelect'),
    rowsSelect: $('#rowsSelect'),
    screenScale: $('#screenScale'),
    screenScaleOut: $('#screenScaleOut'),
    showDots: $('#showDots'),
    showArrows: $('#showArrows'),
    iconSize: $('#iconSize'),
    iconSizeOut: $('#iconSizeOut'),
    iconRadius: $('#iconRadius'),
    iconRadiusOut: $('#iconRadiusOut'),
    iconOpacity: $('#iconOpacity'),
    iconOpacityOut: $('#iconOpacityOut'),
    labelSize: $('#labelSize'),
    labelSizeOut: $('#labelSizeOut'),
    ...
    middleColorRow: $('#middleColorRow'),
    middleColor: $('#middleColor'),
    middlePositionRow: $('#middlePositionRow'),
    middlePosition: $('#middlePosition'),
    middlePositionOut: $('#middlePositionOut'),
    bottomEnabled: $('#bottomEnabled'),
    bottomColorRow: $('#bottomColorRow'),
    bottomColor: $('#bottomColor'),
    wallpaperInput: $('#wallpaperInput'),
    clearWallpaper: $('#clearWallpaper'),
    wallpaperDim: $('#wallpaperDim'),
    wallpaperDimOut: $('#wallpaperDimOut'),
    wallpaperBlur: $('#wallpaperBlur'),
    wallpaperBlurOut: $('#wallpaperBlurOut'),
    exportButton: $('#exportButton'),
    importInput: $('#importInput'),
    siteDialog: $('#siteDialog'),
    siteForm: $('#siteForm'),
    siteDialogTitle: $('#siteDialogTitle'),
    siteUrl: $('#siteUrl'),
    siteTitle: $('#siteTitle'),
    closeSiteDialog: $('#closeSiteDialog'),
    cancelSiteButton: $('#cancelSiteButton'),
    lookupIconButton: $('#lookupIconButton'),
    webImageSearchButton: $('#webImageSearchButton'),
    iconLookupStatus: $('#iconLookupStatus'),
    iconChoices: $('#iconChoices'),
    customIconInput: $('#customIconInput'),
    cropSelectedButton: $('#cropSelectedButton'),
    deleteSiteButton: $('#deleteSiteButton'),
  ...
  // Persistent data is stored in `state`; the remaining variables are transient UI state.
  let state;
  let editing = false;
  let renderSeq = 0;
  ...
  async function iconVisual(item) {
  ...
  async function makeShortcut(item, { folderId = null } = {}) {
    ...
    el.appendChild(await iconVisual(item));
    const lab = document.createElement('div');
    lab.className = 'shortcut-label';
    lab.textContent = item.title || 'Untitled';
    el.appendChild(lab);
    if (editing) {
      const pen = document.createElement('button');
      pen.type = 'button';
      pen.className = 'edit-pencil';
      pen.textContent = '✎';
      pen.title = 'Edit';
  ...
  async function render() {
    ...
    for (let i = 0; i < capacity; i++) {
      ...
      if (item) cell.appendChild(await makeShortcut(item));
        ...
        plus.type = 'button';
        plus.className = 'empty-slot';
        plus.textContent = '+';
        plus.addEventListener('click', () => openAdd(i));
  ...
  async function go(i) {
    ...
    try {
      ...
      // and caused a sub-frame "copy left behind" shift at transition start.
      els.grid.style.visibility = 'hidden';
      state.currentPage = i;
      await save();
      await render();
  ...
  async function moveOutOfFolder(id, folderId) {
    ...
    const emptied = f.items.length === 0;
    if (emptied) {
  ...
  function sync() {
    ...
    els.middleEnabled.checked = s.middleEnabled;
    els.bottomEnabled.checked = s.bottomEnabled;
    els.screenScaleOut.textContent = s.screenScale + '%';
    els.iconSizeOut.textContent = s.iconSize + 'px';
    els.iconRadiusOut.textContent = s.iconRadius + '%';
    els.iconOpacityOut.textContent = s.iconOpacity + '%';
    els.labelSizeOut.textContent = s.labelSize + 'px';
  ...
  async function openEdit(id) {
    ...
    els.siteDialog.dataset.slot = '';
    els.deleteSiteButton.classList.remove('hidden');
    els.iconLookupStatus.textContent = '';
    let current = [];
  ...
  async function findIcons() {
    ...
    if (resp && resp.ok) {
      ...
      if (type.includes('text/html') || type.includes('application/xhtml')) {
        ...
        if (manifestLink) {
          try {
            const mu = new URL(manifestLink, pageUrl);
            if (mu.origin === new URL(pageUrl).origin) {
  ...
  function bindCoreEvents() {
    ...
    fill(els.rowsSelect, 2, 6);

    els.settingsButton.onclick = openSettings;
    els.closeSettings.onclick = closeSettings;
    els.panelScrim.onclick = closeSettings;
    els.prevPage.onclick = () => go(state.currentPage - 1);
    els.nextPage.onclick = () => go(state.currentPage + 1);
  ...
  function bindSiteEditorEvents() {
    els.siteForm.onsubmit = submitSite;
    els.closeSiteDialog.onclick = () => els.siteDialog.close();
    els.cancelSiteButton.onclick = () => els.siteDialog.close();
    els.lookupIconButton.onclick = findIcons;

    els.webImageSearchButton.onclick = () => {
      const raw = els.siteUrl.value.trim();
      let host = raw;
      try {
    ...
    els.cropSelectedButton.onclick = async () => {
      ...
      if (selectedIcon?.kind === 'remote') {
        ...
        const d = await remoteIconToDataUrl(selectedIcon.url);
  ...
  function bindCropEditorEvents() {
    els.closeCropDialog.onclick = () => els.cropDialog.close();
    els.cropCancel.onclick = () => els.cropDialog.close();
    els.cropApply.onclick = applyCrop;
  ...
  function bindGroupAndBackupEvents() {
    ...
    els.cancelDeletePage.onclick = () => els.deletePageDialog.close();
    els.continueDeletePage.onclick = confirmDeleteCurrentPage;
    els.groupEditForm.onsubmit = saveGroup;
    els.closeGroupEditDialog.onclick = () => els.groupEditDialog.close();
    els.cancelGroupEdit.onclick = () => els.groupEditDialog.close();
    els.deleteGroupButton.onclick = deleteGroup;
    els.exportButton.onclick = exportBackup;
    els.importInput.onchange = () => importBackup(els.importInput.files?.[0]);
  }

  function bindGlobalEvents() {
    window.addEventListener('dragover', handleEdgeDrag);
